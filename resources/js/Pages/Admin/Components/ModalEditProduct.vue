<script setup>
    ...
    const productImages = computed(() => 
        props?.product?.product_images?.map(img => `/storage/${img.image}`) || []
    );

    const previews = ref(productImages.value);
    ...
    const submit = (id) => {
        ...
        formData = {...form, _method: 'PUT'};

        router.post(`/admin/products/${id}`, formData, {
            forceFormData: true,
            onSuccess: () => {
  ...
  >
            ...
            <form action="#">
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <!-- Image previews -->
                        <div v-if="previews.length || newPreviews.length" class="mt-2 flex gap-2 flex-wrap">
                        ...
                        <div v-for="(src, index) in newPreviews" :key="index" class="relative">
                            <img :src="src" class="w-16 h-16 object-cover rounded" />
                            <button @click="removeNewImage(index)" class="absolute top-0 right-0 text-red-500">✕</button>
                        </div>
                        </div>
                    </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="submit(product.id)" :disabled="form.processing">
          Confirm
        </el-button>
      </div>
    </template>
