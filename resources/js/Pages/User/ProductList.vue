<script setup>
    ...
    import HeroBento from './Components/Hero.vue';
    ...
    const filters = computed(() => [
        {
            ...
            options: props.brands.map(brand => ({
                value: brand.id,
                label: brand.name,
                checked: formFilters.brands.includes(brand.id),
            }))
        },
        {
            id: 'categories',
            ...
            options: props.categories.map(category => ({
                value: category.id,
                label: category.name,
                checked: formFilters.categories.includes(category.id),
            }))
        },
    ])
    ...
    function toggleFilter(sectionId, value) {
        ...
        const section = formFilters[sectionId]
        const index = section.indexOf(value)
        if (index === -1) {
            section.push(value)
        } else {
    ...
    watch(formFilters, (newFilters) => {
        const isEmpty = 
            ...
            !newFilters.prices.from && 
            !newFilters.prices.to
        ...
        router.get(route('product.index'), {
            ...
            prices: (newFilters.prices.from || newFilters.prices.to) ? newFilters.prices : undefined,
        }, {
            preserveScroll: true,
            preserveState: true,
        })
...
<template>
    ...
    <div class="bg-white">
        ...
        <main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <h1 class="text-4xl font-bold tracking-tight text-gray-900">Latest Trends</h1>
            ...
            <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                ...
                <div class="flex items-center gap-2">
                    <div class="flex-1">
                        ...
                        <input 
                            type="number" 
                            v-model="formFilters.prices.from"
                            min="0"
                            placeholder="$ Min"
                            class="w-full rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        />
                    </div>
                ...
                <Disclosure as="div" v-for="section in filters" :key="section.id" class="border-b border-gray-200 py-6" v-slot="{ open }">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex h-5 shrink-0 items-center">
                            <div class="group grid size-4 grid-cols-1">
                            <input @change="toggleFilter(section.id, option.value)" :id="`filter-${section.id}-${optionIdx}`" :value="option.value" type="checkbox" :checked="option.checked" class="col-start-1 row-start-1 appearance-none rounded-sm border border-gray-300 bg-white checked:border-indigo-600 checked:bg-indigo-600 indeterminate:border-indigo-600 indeterminate:bg-indigo-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 disabled:border-gray-300 disabled:bg-gray-100 disabled:checked:bg-gray-100 forced-colors:appearance-auto" />
                            <svg class="pointer-events-none col-start-1 row-start-1 size-3.5 self-center justify-self-center stroke-white group-has-disabled:stroke-gray-950/25" viewBox="0 0 14 14" fill="none">
                                <path class="opacity-0 group-has-checked:opacity-100" d="M3 8L6 11L11 3.5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path class="opacity-0 group-has-indeterminate:opacity-100" d="M3 7H11" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                ...
                <button @click.prevent="clearFilters" type="button"
                    class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-700">
                    Clear Filters
                </button>
                </form>
                ...
                <!-- Your content -->
                 ...
                 <paginationLinks :products="products"/>
